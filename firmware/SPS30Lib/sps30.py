"""
SPS30 Library - Synchronous version
IMPORTANT CHANGE: SYNCHRONOUS reads (no separate thread)
- Avoids I2C bus collisions with other sensors
- get_measurement() reads directly from the sensor
- Automatic retries if a read fails
...
from SPS30Lib.i2c.i2c import I2C
...
class SPS30:
    ...
    def __init__(self, bus: int = 1, address: int = 0x69, sampling_period: int = 1, logger: str = None, retries: int = 3):
        """
        Initialise the SPS30 sensor (SYNCHRONOUS VERSION).

        Args:
            bus: I2C bus (default 1)
            address: I2C address of the sensor (default 0x69)
            sampling_period: Unused in the synchronous version, kept for compatibility
            logger: Logger name used to record events
            retries: Number of retries if a read fails (default 3)
        """
        self.logger = None
        if logger:
            self.logger = logging.getLogger(logger)

        self.sampling_period = sampling_period  # Kept for compatibility
    ...
    def get_stats(self) -> dict:
        """Return statistics of reads and errors."""
        stats = self.__stats.copy()
        if stats["total_reads"] > 0:
            stats["success_rate"] = round(
                (stats["successful_reads"] / stats["total_reads"]) * 100, 1
            )
        else:
    ...
    def read_data_ready_flag(self) -> bool:
        ...
        if self.crc_calc(data[:2]) != data[2]:
            if self.logger:
                self.logger.warning(
                    "SPS30: 'read_data_ready_flag' CRC mismatched! " +
                    f"Data: {data[:2]} " +
                    f"Calculated: {self.crc_calc(data[:2])} " +
                    f"Expected: {data[2]}")
            return False
        ...
        for block, (pm) in enumerate(category):
            ...
            for i in range(0, SIZE_FLOAT, PACKET_SIZE):
                ...
                if self.crc_calc(data[offset:offset+2]) != data[offset+2]:
                    if self.logger:
                        self.logger.warning(
                            "SPS30: CRC error en mass_density " +
                            f"[{pm}] Data: {data[offset:offset+2]}")
                    self.__stats["crc_errors"] += 1
        ...
        for block, (pm) in enumerate(category):
            ...
            for i in range(0, SIZE_FLOAT, PACKET_SIZE):
                ...
                if self.crc_calc(data[offset:offset+2]) != data[offset+2]:
                    if self.logger:
                        self.logger.warning(
                            "SPS30: CRC error en particle_count " +
                            f"[{pm}] Data: {data[offset:offset+2]}")
                    self.__stats["crc_errors"] += 1
    ...
    def __particle_size_measurement(self, data: list) -> float:
        ...
        for i in range(0, SIZE_FLOAT, PACKET_SIZE):
            if self.crc_calc(data[i:i+2]) != data[i+2]:
                if self.logger:
                    self.logger.warning(
                        "SPS30: CRC error en particle_size " +
                        f"Data: {data[i:i+2]}")
                self.__stats["crc_errors"] += 1
    ...
    def __read_single_measurement(self) -> dict:
        """
        Read a measurement from the sensor synchronously.
        Return a dictionary with data, or {} on failure.
        """
        try:
            # Wait for the data to be ready (max 5 attempts)
            for _ in range(5):
                if self.read_data_ready_flag():
                    break
                sleep(0.2)
            else:
                return {}
            ...
            if self.logger:
                self.logger.warning(f"SPS30: {type(e).__name__}: {e}")
            self.__stats["i2c_errors"] += 1
            return {}

    def start_measurement(self) -> None:
        """Start the sensor's measurement mode."""
        data_format = {
            "IEEE754_float": 0x03,
            "unsigned_16_bit_integer": 0x05
        }

        data = CMD_START_MEASUREMENT.copy()  # IMPORTANT: copy so the original is not modified
        data.extend([data_format["IEEE754_float"], 0x00])
        data.append(self.crc_calc(data[2:4]))
        self.i2c.write(data)
        sleep(0.05)
        # Does NOT start a thread - the read will be synchronous

    def get_measurement(self) -> dict:
        """
        Read a measurement from the sensor SYNCHRONOUSLY.
        Try up to 'retries' times if errors occur.
        Return a dictionary with data, or {} on failure.
        """
        self.__stats["total_reads"] += 1

        for attempt in range(self.retries):
            result = self.__read_single_measurement()

            if result and result.get("sensor_data", {}).get("mass_density"):
                self.__stats["successful_reads"] += 1
                return result
    ...
    def stop_measurement(self) -> None:
        """Stop the measurement mode and close I2C."""
